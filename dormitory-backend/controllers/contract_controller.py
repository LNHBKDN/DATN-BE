...
# Custom JSON encoder để xử lý datetime và các kiểu dữ liệu khác
...
# Lấy chi tiết hợp đồng theo ID (Admin hoặc User sở hữu)
    ...
    try:
        ...
        if isinstance(identity, str):
            try:
                ...
                return jsonify({'message': 'Token có định dạng không hợp lệ'}), 401
        else:
